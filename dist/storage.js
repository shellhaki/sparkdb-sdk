...
export class Storage {
    ...
    async createBucket(name, options = {}) {
        return this.spark.call('POST', '/api/sdk/buckets', {
            json: { name, public: options.public ?? true }
        });
    }
    /** List your buckets. */
    async listBuckets() {
        const res = await this.spark.call('GET', '/api/sdk/buckets');
        return res.buckets ?? [];
    }
...
export class BucketHandle {
    constructor(spark, storage, idOrName) {
        this.spark = spark;
    ...
    async resolveId() {
        ...
        if (!match)
            throw new Error(`SparkDB bucket "${this.idOrName}" not found.`);
        return match.id;
    }
    /**
    ...
    async upload(input) {
        ...
        return this.spark.call('POST', `/api/sdk/buckets/${id}/upload`, { body: toFormData(input) });
    }
    /** List the files in this bucket. */
    async list() {
        const id = await this.resolveId();
        const res = await this.spark.call('GET', `/api/sdk/buckets/${id}/objects`);
        return res.objects ?? [];
    }
...
function toFormData(input) {
    ...
    if (input instanceof Blob) {
        const filename = 'name' in input && typeof input.name === 'string' ? input.name : 'file';
        form.append('file', input, filename);
        return form;
    }
    const { data, filename, contentType } = input;
    const blob = data instanceof Blob ? data : new Blob([data], contentType ? { type: contentType } : undefined);
